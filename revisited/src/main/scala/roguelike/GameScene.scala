...
object GameScene extends Scene[Unit, Model, ViewModel]:
  ...
  def updateModel(
    ...
    // Window close keys
    case KeyboardEvent.KeyUp(Key.ESCAPE) | KeyboardEvent.KeyUp(Key.SHIFT) |
        KeyboardEvent.KeyUp(Key.CTRL) | KeyboardEvent.KeyUp(Key.ALT)
    ...
    case KeyboardEvent.KeyDown(Key.UP_ARROW)
        if model.currentState.showingHistory =>
      Outcome(
        model.copy(
    ...
    case KeyboardEvent.KeyDown(Key.DOWN_ARROW)
      ...
      Outcome(
        model.copy(
          historyViewer =
            model.historyViewer.scrollDown(model.messageLog.logLength)
        )
      ).addGlobalEvents(GameEvent.Redraw)

    // Inventory window
    case KeyboardEvent.KeyDown(Key.UP_ARROW)
        if model.currentState.showingInventory =>
      Outcome(
        model.copy(
    ...
    case KeyboardEvent.KeyDown(Key.DOWN_ARROW)
      ...
      Outcome(
        model.copy(
          inventoryWindow = model.inventoryWindow.scrollDown(
            model.player.inventory.items.length
          )
        )
      ).addGlobalEvents(GameEvent.Redraw)
    ...
    // Drop window
    case KeyboardEvent.KeyDown(Key.UP_ARROW)
        if model.currentState.showingDropMenu =>
      Outcome(
        model.copy(
    ...
    case KeyboardEvent.KeyDown(Key.DOWN_ARROW)
      ...
      Outcome(
        model.copy(
          dropWindow =
            model.dropWindow.scrollDown(model.player.inventory.items.length)
        )
      ).addGlobalEvents(GameEvent.Redraw)
    ...
    case KeyboardEvent.KeyUp(Key.KEY_G)
        if model.currentState.isRunning && model.player.isAlive =>
      model.pickUp

    case KeyboardEvent.KeyUp(Key.PERIOD)
        if model.currentState.isRunning && model.player.isAlive =>
    ...
    case KeyboardEvent.KeyUp(Key.KEY_I)
    ...
    case KeyboardEvent.KeyUp(Key.KEY_C)
        if model.currentState.isRunning || model.currentState.showingCharacter =>
      Outcome(model.toggleCharacter)
        .addGlobalEvents(GameEvent.Redraw)

    // Look Around
    case KeyboardEvent.KeyUp(Key.FORWARD_SLASH)
  ...
  def updateViewModel(
    ...
    case GameEvent.Redraw | GameEvent.PlayerTurnEnd
        if viewModel.lastRedraw < context.running =>
      viewModel.redrawTerminal(model, context.running)
