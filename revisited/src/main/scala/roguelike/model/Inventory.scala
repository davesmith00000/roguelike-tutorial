  ...
  def add(item: Item): Outcome[(Inventory, Boolean)] =
    ...
    if newItems.length > capacity then
      ...
      Outcome(
        ...
        GameEvent.Log(
          Message(s"You picked up the ${item.name}!", RGB(0.0, 0.85, 0.0))
        ),
    ...
    items.lift(itemAt) match
      ...
      case Some(Item(_, potion @ Consumable.HealthPotion(_))) =>
        Consumable.useHealthPotion(potion, player).map { attempt =>
          if attempt._2 then
            (this.copy(items = remove(itemAt, items)), attempt._1)
          else (this, player)
        }

      case Some(Item(_, scroll @ Consumable.LightningScroll(_, _))) =>
        Consumable.useLightningScroll(scroll, player, visibleHostiles).map {
          consumed =>
  ...
  def consumeTargeted(
    ...
    items.lift(itemAt) match
      ...
      case Some(Item(_, scroll @ Consumable.FireBallScroll(_, _))) =>
        Consumable.useFireballScroll(scroll, player, target, hostiles).map {
          consumed =>
            if consumed then (this.copy(items = remove(itemAt, items)), player)
            else (this, player)
        }
      ...
      case item @ Some(_) =>
        ...
        Outcome((next, item))
          .addGlobalEvents(
            GameEvent.Log(
              Message(
                s"You dropped the ${item.map(_.name).getOrElse("<name missing!>")}.",
                RGB.White
              )
            ),
            GameEvent.PlayerTurnEnd
