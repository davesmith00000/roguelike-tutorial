...
object Consumable:
  ...
  final case class LightningScroll(damage: Int, maximumRange: Int)
      extends Consumable:
  ...
  final case class LeatherArmor(id: BindingKey, defenseBonus: Int)
      extends Armour:
    val name: String  = LeatherArmor.name
    val tile: MapTile = MapTile(Tile.`[`, RGB.fromColorInts(139, 69, 19))
  object LeatherArmor:
    val name: String = "Leather Armor"
    def create(dice: Dice): LeatherArmor =
      LeatherArmor(BindingKey.fromDice(dice), 1)
  ...
  def useHealthPotion(
    ...
    if amountRecovered <= 0 then
      ...
      val msg =
        Message(
          s"You consume the ${healthPotion.name}, and recover $amountRecovered",
  ...
  def useLightningScroll(
    ...
    findClosest(hostiles, scroll.maximumRange + 1, None) match
      ...
      case Some(target) =>
        val msg =
          s"A lighting bolt strikes the ${target.name} with a loud thunder, for ${scroll.damage} damage!"
        Outcome(true)
          .addGlobalEvents(
  ...
  def useFireballScroll(
    ...
    val events =
      hostiles
        .filter(_.position.distanceTo(target.position).toInt <= scroll.radius)
        .flatMap { h =>
          List(
            GameEvent.Log(
              Message(
