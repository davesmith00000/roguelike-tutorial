...
final case class Player(
  ...
  def tile: MapTile = if isAlive then MapTile(Tile.`@`, RGB.Magenta)
  else MapTile(Tile.`@`, RGB.Red)
  ...
  def consumeTargetted(
      itemAt: Int,
      target: Hostile,
      visibleHostiles: List[Hostile]
  ): Outcome[Player] =
    inventory
  ...
  def drop(
      itemAt: Int,
      worldItems: List[Item]
  ): Outcome[(Player, Option[Item])] =
    if worldItems.exists(
        _.position == position
        ...
        .flatMap {
          ...
          case (inv, Some(item @ Item(_, w: Weapon))) =>
            equipment.unequip(w).map { eqip =>
              (
                this.copy(inventory = inv, equipment = eqip),
                Option(item.moveTo(position))
              )
            }
        ...
        inventory.add(item).map { case (inv, accepted) =>
          if accepted then
            (
              ...
              worldItems.filterNot(_.position == position)
            )
  ...
  def bump(amount: Point, gameMap: GameMap): Outcome[Player] =
    gameMap.hostiles.collectFirst {
      case e: Hostile if e.position == position + amount && e.blocksMovement =>
        e
    } match
      case None =>
...
final case class Troll(
  ...
  def tile: MapTile =
    if isAlive then MapTile(Tile.`T`, RGB.fromColorInts(0, 127, 0))
    else MapTile(Tile.`%`, RGB.Magenta)
  val blocksMovement: Boolean = isAlive
