...
final case class Orc(id: Int, position: Point, isAlive: Boolean, fighter: Fighter, movePath: List[Point])
    extends Hostile:
  def tile: MapTile =
    if isAlive then MapTile(Tile.`o`, RGB.fromColorInts(63, 127, 63)) else MapTile(Tile.`o`, RGB.Red)
...
final case class Troll(id: Int, position: Point, isAlive: Boolean, fighter: Fighter, movePath: List[Point])
    extends Hostile:
  def tile: MapTile =
    if isAlive then MapTile(Tile.`T`, RGB.fromColorInts(0, 127, 0)) else MapTile(Tile.`T`, RGB.Red)
  val blocksMovement: Boolean = isAlive
